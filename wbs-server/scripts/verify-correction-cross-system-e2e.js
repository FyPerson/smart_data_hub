...
(async () => {
  ...
  const cb = await reqMultipartCreate( {
    ...
    error_proof_note: '错误截图见附件', cross_system: true, system2: { source_system: 'HRD', location_info: '系统2：同字段错', correction_count: 3 },
  }, ADMIN);
  ...
  ok(cRow.source_system === 'HRD' && cRow.location_info === '系统2：同字段错' && cRow.correction_count === 1, '① 子单系统字段=system2（HRD/修正方式）+ single 主单→子单 cc 强制 1（传入 3 被忽略，用户优化条数分流）');
  ...
  // ② 禁直派 + system2 校验
  const dA = await reqMultipartCreate( { source_system: 'BMS', location_info: 'x', correction_type: 'single', oa_number: oa(), requesters: [{ name: '甲' }], cross_system: true, system2: { source_system: 'HRD', location_info: 'y' }, assigned_to: 99 }, ADMIN);
  ok(dA.status === 400 && dA.body.code === 'CROSS_SYSTEM_NO_DIRECT_ASSIGN', '② 跨系统 + assigned_to → 400 CROSS_SYSTEM_NO_DIRECT_ASSIGN');
  const dR = await reqMultipartCreate( { source_system: 'BMS', location_info: 'x', correction_type: 'single', oa_number: oa(), requesters: [{ name: '甲' }], cross_system: true, system2: { source_system: 'HRD', location_info: 'y' }, relay_notified_user_id: 7 }, ADMIN);
  ok(dR.status === 400 && dR.body.code === 'CROSS_SYSTEM_NO_DIRECT_ASSIGN', '② 跨系统 + relay → 400 CROSS_SYSTEM_NO_DIRECT_ASSIGN');
  const dL = await reqMultipartCreate( { source_system: 'BMS', location_info: 'x', correction_type: 'single', oa_number: oa(), requesters: [{ name: '甲' }], cross_system: true, system2: { source_system: 'HRD' } }, ADMIN);
  ...
  const dF = await reqMultipartCreate( { source_system: 'BMS', location_info: 'x', correction_type: 'single', oa_number: oa(), requesters: [{ name: '甲' }], system2: { source_system: 'HRD', location_info: 'y' } }, ADMIN);   // 传 system2 但无 cross_system flag
  ...
  const cL = await reqJson('POST', `/api/corrections/${childId}/link-new`, { source_system: '客户报销平台', location_info: 'z' }, ADMIN);
  ...
  const ln = await reqJson('POST', `/api/corrections/${soloId}/link-new`, { source_system: 'HRD', location_info: '追加系统', correction_count: 5 }, ADMIN2);   // soloId 由 ADMIN(1) 建，link-new 由 ADMIN2(2) 做
  ...
  ok(Number(newRow.correction_group_id) === soloId && newRow.requester_name === '独立主' && newRow.requester_phone === '13700000007' && newRow.source_system === 'HRD',
  ...
  const lnT = await reqJson('POST', `/api/corrections/${soloId}/link-new`, { source_system: '客户报销平台', location_info: 'z' }, ADMIN);
  ...
  const lnN = await reqJson('POST', `/api/corrections/${s2.body.id}/link-new`, { source_system: 'HRD', location_info: 'z' }, ADMIN);
  ...
  const okLn = await reqJson('POST', `/api/corrections/${s3.body.id}/link-new`, { source_system: 'HRD', location_info: 'z' }, ADMIN);
  ok(okLn.status === 200 && okLn.body.id, '⑧ M-2：link-new 追加不同系统(HRD) → 放行');
  ...
  const cc_xs0 = await reqMultipartCreate( { source_system: 'BMS', location_info: 'x', correction_type: 'batch', oa_number: oa(), correction_count: 5, reason: '跨系统batch缺子条数测试原因', requesters: [{ name: '甲' }], cross_system: true, system2: { source_system: 'HRD', location_info: 'y' } }, ADMIN);
  ...
  const cc_xs1 = await reqMultipartCreate( { source_system: 'BMS', location_info: 'x', correction_type: 'batch', oa_number: oa(), correction_count: 5, reason: '跨系统batch子条数1测试原因', requesters: [{ name: '甲' }], cross_system: true, system2: { source_system: 'HRD', location_info: 'y', correction_count: 1 } }, ADMIN);
  ok(cc_xs1.status === 400 && cc_xs1.body.code === 'LINKED_BATCH_COUNT_MIN', '⑨ 跨系统 batch system2 cc=1 → 400 LINKED_BATCH_COUNT_MIN');
  const cc_xs = await reqMultipartCreate( { source_system: 'BMS', location_info: 'x', correction_type: 'batch', oa_number: oa(), correction_count: 5, reason: '跨系统batch条数分流测试原因', requesters: [{ name: '甲', phone: '13700000021' }], cross_system: true, system2: { source_system: 'HRD', location_info: 'y', correction_count: 3 } }, ADMIN);
