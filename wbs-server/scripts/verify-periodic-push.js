...
//
// ⚠️ 改造（2026-07-06）：验收发现钉钉详情接口(topapi/v2/user/get)未开通 qyapi_get_member 权限（errcode 60011），
//   用户拍板改用数据协作模块成熟模式——**姓名由 admin 录入、不向钉钉查**，钉钉侧只用 getUserIdByMobile
//   （phone→userid，已授权）判"能否送达"。故本脚本删去 getUserDetail mock + active/在职/DETAIL_* 断言，
//   改测：姓名录入透传（preview/confirm 用录入 name 而非钉钉）+ phone→userid + H-1 userid 硬 gate 一致性。
//   idempotency 三态 / reaper([10e]) / force supersede([10d]) 逻辑未动，断言全保留（应零回归）。
//
// 断言覆盖：
//   [单元层] recipients.js resolvePushRecipient：手机号格式非法/反查查不到/查到多条(逗号分隔)/成功解析
//     (只返 userid + 钉钉ID后四位，无姓名/在职/部门)；getUserDetail 已从 module.exports 删除。
//   [端点层] 可推送前置(§6.1)：success可推 / empty_result默认拒需confirm_empty / failed拒 / running拒 /
//     file_status非present拒；收件人核对预览(录入姓名透传+不落库)；确认发送：批量部分反查失败→skipped继续发
...
require.cache[dtPath] = {
  ...
  exports: {
    ...
    // 手机号 → userid（改造后唯一的钉钉核对调用；按号码分派固定场景，供单元 + 端点测试复用）
    getUserIdByMobile: async (token, phone) => {
      if (phone === '13800000001') return 'uid0001';
      if (phone === '13800000002') return 'uid0002';
      if (phone === '13800000004') return 'uid_invalid_staff'; // 会被 sendFileToUser 判 invalidStaffIdList
      if (phone === '13800000005') return 'uid0001,uid9999'; // 多组织账号多值 → MULTIPLE_MATCHES
      if (phone === '13800000007') return 'uid0007'; // 正常账号，用于 H-1 快照比对/去重
...
// ⚠️ 改造守卫：删除 getUserDetail/topapi 后，本模块不应再有任何 fetch 调用。用抛错的 fetch 兜底——
//   若将来有人重新引入详情 HTTP 调用，测试会在此立即失败暴露（防回归）。
const originalFetch = global.fetch;
global.fetch = async (url) => {
  throw new Error('本模块改造后不应再有任何 fetch 调用（getUserDetail/topapi 已删除）: ' + url);
};
...
async function unitTests() {
  ...
  assert.ok(!('name' in r), '改造后不再返回 name（姓名由 admin 录入，不向钉钉查）');
  assert.ok(!('deptIds' in r), '改造后不再返回 deptIds（已删 getUserDetail 详情反查）');
  assert.ok(!('active' in r) && !('activeRaw' in r), '改造后不再返回在职状态（钉钉详情接口无权限）');
  ok('正常反查：手机号 → userid + 钉钉ID后四位（无姓名/在职/部门——getUserDetail 依赖已彻底删除）');

  // getUserDetail 已从 module.exports 彻底删除（改造核心断言）
  assert.strictEqual(typeof recipients.getUserDetail, 'undefined', 'recipients.getUserDetail 已删除（module.exports 不含 getUserDetail）');
  ok('改造断言：recipients.getUserDetail 已从 module.exports 删除（topapi/v2/user/get 无权限依赖已清除）');
  ...
  // H-new 单元：evaluatePushIdempotency 纯判定（require 真实 _internals，逻辑未动应零回归）
  ...
  ok('H-new 单元：evaluatePushIdempotency——pending→PUSH_IN_PROGRESS(force也拦) / success非force→ALREADY_PUSHED / success+force→放行 / pending 优先（逻辑未动）');
  ...
  ok('M（集成点3）单元：evaluatePushIdempotency——WRITE_FAILED 标记的 pending → NEEDS_RECONCILE(force 可覆盖) / 无标记 pending → 仍 PUSH_IN_PROGRESS(force 不可穿透) / 混合时真在途优先（逻辑未动）');

  // 改造断言：name/last4 drift 的 compareRecipientSnapshot/normalizeForCompare 已删除（userid 硬 gate 内联到 confirm）
  assert.strictEqual(typeof I2.compareRecipientSnapshot, 'undefined', 'compareRecipientSnapshot 已删除（改造后 userid 硬 gate 内联，无 name/last4 drift）');
  assert.strictEqual(typeof I2.normalizeForCompare, 'undefined', 'normalizeForCompare 已删除（无 drift 归一化比对需求）');
  ok('改造断言：compareRecipientSnapshot / normalizeForCompare 已从 _internals 删除（drift 概念消失，userid 硬 gate 内联）');

  // M-3（codex v1.105.1）单元：findPhoneNameConflict——同号不同名 → 返回冲突手机号；同号同名/不同号/trim后一致 → null
  assert.strictEqual(I2.findPhoneNameConflict([{ name: '张三', phone: '13800000001' }, { name: '李四', phone: '13800000001' }]), '13800000001', '同一手机号填了不同姓名 → 返回冲突手机号');
  assert.strictEqual(I2.findPhoneNameConflict([{ name: '张三', phone: '13800000001' }, { name: '张三', phone: '13800000001' }]), null, '同号同名 → 无冲突（返 null）');
  assert.strictEqual(I2.findPhoneNameConflict([{ name: '张三', phone: '13800000001' }, { name: '李四', phone: '13900000002' }]), null, '不同号不同名 → 无冲突');
  assert.strictEqual(I2.findPhoneNameConflict([{ name: ' 张三 ', phone: '13800000001' }, { name: '张三', phone: ' 13800000001 ' }]), null, '姓名 trim + phone 归一化后一致 → 无冲突');
  ok('M-3 单元：findPhoneNameConflict——同号不同名→冲突手机号 / 同号同名→null / 不同号→null / trim+归一化后一致→null');

  // dedupeByNormalizedPhone 对象数组去重（codex v1.105.1 HIGH refuted 实证）：按 item.phone 去重、保留整个对象
  {
    const ded = I2.dedupeByNormalizedPhone([{ name: '张三', phone: '13800000001' }, { name: '李四', phone: '13900000002' }, { name: '张三', phone: ' 13800000001 ' }]);
    assert.strictEqual(ded.kept.length, 2, '对象数组按 phone 去重：3 项 2 手机号 → 保留 2（非 [object Object] 合并成 1）');
    assert.strictEqual(ded.dropped, 1, '重复手机号丢弃 1 个');
    assert.strictEqual(ded.kept[0].name, '张三', '保留整个对象（含录入姓名）、非归一化字符串');
  }
  ok('dedupeByNormalizedPhone 单元：对象数组按 item.phone 去重、保留整个对象（codex HIGH refuted 实证——helper 本就 typeof item==="string"?item:item.phone 双模式）');
}
...
async function endpointTests() {
  ...
  // ── [1] 可推送前置：success + file present → 可推；预览回显录入姓名（非钉钉）──
  const runSuccess = await seedRun(taskId, { status: 'success', fileStatus: 'present', withFile: true });
  let r = await call('POST', `/api/periodic-tasks/runs/${runSuccess}/push/preview`, adminTok, { recipients: [{ name: '张三录入', phone: '13800000001' }] });
  assert.strictEqual(r.status, 200);
  assert.strictEqual(r.body.items[0].ok, true);
  assert.strictEqual(r.body.items[0].name, '张三录入', '预览回显姓名 = admin 录入值（不向钉钉查）');
  assert.strictEqual(r.body.items[0].last4, '0001', '预览带 userid 后四位供核对可送达');
  assert.ok(r.body.items[0].userid, '预览带 userid 供 confirm 快照');
  ok(`前置：success+file present → 预览通过（run #${runSuccess}），姓名透传录入值+userid可送达`);

  // ── [2] 可推送前置：empty_result 默认拒 ──
  const runEmpty = await seedRun(taskId, { status: 'empty_result', fileStatus: 'present', withFile: true });
  r = await call('POST', `/api/periodic-tasks/runs/${runEmpty}/push/preview`, adminTok, { recipients: [{ name: '张三', phone: '13800000001' }] });
  ...
  r = await call('POST', `/api/periodic-tasks/runs/${runEmpty}/push/preview`, adminTok, { recipients: [{ name: '张三', phone: '13800000001' }], confirm_empty: true });
  assert.strictEqual(r.status, 200);
  ...
  r = await call('POST', `/api/periodic-tasks/runs/${runFailed}/push/preview`, adminTok, { recipients: [{ name: '张三', phone: '13800000001' }] });
  assert.strictEqual(r.status, 409);
  ...
  r = await call('POST', `/api/periodic-tasks/runs/${runRunning}/push/preview`, adminTok, { recipients: [{ name: '张三', phone: '13800000001' }] });
  assert.strictEqual(r.status, 409);
  ...
  r = await call('POST', `/api/periodic-tasks/runs/${runCleaned}/push/preview`, adminTok, { recipients: [{ name: '张三', phone: '13800000001' }] });
  assert.strictEqual(r.status, 409);
  assert.strictEqual(r.body.code, 'FILE_NOT_PRESENT');
  ok('前置：success 但文件已清理 → 409 FILE_NOT_PRESENT（方案 §4.4）');

  // ── [6] 收件人预览：混合三态 + 录入姓名全透传 + 不落库（push 表应仍为 0 行）──
  const countBefore = await get(`SELECT COUNT(*) AS c FROM periodic_task_pushes`);
  r = await call('POST', `/api/periodic-tasks/runs/${runSuccess}/push/preview`, adminTok, { recipients: [
    { name: '甲', phone: '13800000001' },   // ok
    { name: '乙', phone: '13800000005' },   // MULTIPLE_MATCHES
    { name: '丙', phone: '13800009999' },   // NOT_FOUND
  ] });
  assert.strictEqual(r.status, 200);
  assert.strictEqual(r.body.items.length, 3);
  assert.strictEqual(r.body.items[0].ok, true);
  assert.strictEqual(r.body.items[0].name, '甲', 'ok 项回显录入姓名');
  assert.strictEqual(r.body.items[1].reason, 'MULTIPLE_MATCHES');
  assert.strictEqual(r.body.items[1].name, '乙', '失败项也回显录入姓名（供 admin 定位是哪个人的号有问题）');
  assert.strictEqual(r.body.items[2].reason, 'NOT_FOUND');
  assert.strictEqual(r.body.items[2].name, '丙', '失败项也回显录入姓名');
  ...
  ok('预览：ok/多匹配/查不到 三态一次返回 + 录入姓名全透传（含失败项），且不写 push 表（无副作用）');

  // ── [6b] 预览入参校验：缺姓名/缺手机号 → 400 INVALID_RECIPIENT_INPUT ──
  r = await call('POST', `/api/periodic-tasks/runs/${runSuccess}/push/preview`, adminTok, { recipients: [{ phone: '13800000001' }] });
  assert.strictEqual(r.status, 400);
  assert.strictEqual(r.body.code, 'INVALID_RECIPIENT_INPUT', '缺姓名 → 400 INVALID_RECIPIENT_INPUT');
  r = await call('POST', `/api/periodic-tasks/runs/${runSuccess}/push/preview`, adminTok, { recipients: [{ name: '张三' }] });
  assert.strictEqual(r.status, 400);
  assert.strictEqual(r.body.code, 'INVALID_RECIPIENT_INPUT', '缺手机号 → 400 INVALID_RECIPIENT_INPUT');
  ok('预览入参校验：每项须含姓名+手机号，缺一 → 400 INVALID_RECIPIENT_INPUT（姓名是防错发人工核对锚点）');

  // 快照构造 helper（H-1）：走真实 preview 拿回 {phone,userid,name(录入),last4}——confirm 必须携带这份期望快照。
  //   同时断言 preview 回显的 name 恒等于录入值（姓名录入透传的端到端证据）。
  async function snap(runId, phone, name) {
    const nm = name || ('录入-' + String(phone).slice(-4));
    const pr = await call('POST', `/api/periodic-tasks/runs/${runId}/push/preview`, adminTok, { recipients: [{ name: nm, phone }], confirm_empty: true });
    const item = (pr.body.items || []).find((i) => i.phone === recipients.normalizePhone(phone));
    assert.ok(item && item.ok, `snap(${phone}) 预览应 ok`);
    assert.strictEqual(item.name, nm, `snap(${phone}) 预览回显姓名 = 录入值「${nm}」（非钉钉）`);
  ...
  // ── [7] 确认发送（H-3 两阶段 + H-1 userid 硬 gate）：两个合法收件人 → 均 success，留痕 recipient_name_snapshot=录入姓名 ──
  const run7 = await freshRun();
  const snap001 = await snap(run7, '13800000001', '张三');
  const snap007 = await snap(run7, '13800000007', '孙七');
  ...
  assert.strictEqual(rows7.find((p) => p.phone_snapshot === '13800000001').recipient_name_snapshot, '张三', '成功行留痕 recipient_name_snapshot = admin 录入姓名');
  assert.strictEqual(rows7.find((p) => p.phone_snapshot === '13800000007').recipient_name_snapshot, '孙七', '另一行留痕也 = 录入姓名');
  ok('确认发送（H-1 userid 硬 gate + H-3 两阶段）：2 收件人 → 均 success，留痕 pending→success + recipient_name_snapshot 存录入姓名');
  ...
  ok('H-1：confirm 缺 recipients 快照 → 400 RECIPIENTS_REQUIRED；快照字段不全（phone/userid/name/last4）→ 400 INVALID_RECIPIENT_SNAPSHOT');

  // ── [9] H-1 userid 硬 gate：userid 不一致 → RECIPIENT_MISMATCH 拒发；不阻断其余 ──
  const run9 = await freshRun();
  const goodSnap = await snap(run9, '13800000001', '张三');
  // userid 篡改（phone7 却带 uid0001，与后端重查的 uid0007 不符）→ 硬 gate 拒发
  const tamperedUid = { ...(await snap(run9, '13800000007', '孙七')), userid: 'uid0001' };
  ...
  ok('H-1 userid 硬 gate：confirm 重解 userid 与快照不一致 → RECIPIENT_MISMATCH 拒发该号、其余照发');

  // ── [9b] 姓名是录入静态值、不做 drift 校验：快照 name 任意改动仍照发（userid 一致），无 snapshot_warning ──
  const run9b = await freshRun();
  const anyNameSnap = { ...(await snap(run9b, '13800000001', '张三')), name: '随便改个名' };
  r = await call('POST', `/api/periodic-tasks/runs/${run9b}/push/confirm`, adminTok, { recipients: [anyNameSnap] });
  assert.strictEqual(r.status, 200);
  assert.strictEqual(r.body.results[0].status, 'success', 'userid 一致 → 照发（姓名不参与 gate）');
  assert.ok(!('snapshot_warning' in r.body.results[0]), '改造后无 name/last4 drift 概念 → 响应不含 snapshot_warning');
  const rows9b = await all(`SELECT recipient_name_snapshot FROM periodic_task_pushes WHERE run_id = ?`, [run9b]);
  assert.strictEqual(rows9b[0].recipient_name_snapshot, '随便改个名', '留痕 recipient_name_snapshot = confirm 携带的录入姓名（原样存，不与钉钉比对）');
  ok('姓名录入语义：name 是 admin 录入静态值、不做 drift 校验（userid 一致即照发），无 snapshot_warning，留痕存录入值');
  ...
  assert.strictEqual(r.status, 200);
  ok('H-3②：已推成功的 run 再 confirm → 409 ALREADY_PUSHED（防重发敏感文件）；force=true 显式重发放行（逻辑未动）');

  // ── [10b] H-new 并发竞态：run 已有 pending（首个 confirm 进行中）→ 第二个 confirm 被拦 PUSH_IN_PROGRESS ──
  const runP = await freshRun();
  const snapP = await snap(runP, '13800000001', '张三');
  ...
  ok('H-new：run 已有 pending → 第二个 confirm 返 409 PUSH_IN_PROGRESS（force 也拦）+ 事务 ROLLBACK 不新增留痕（并发竞态闭合，逻辑未动）');

  // ── [10c] M-new 阶段三回写失败：UPDATE pending→success 影响 0 行 → 返回 write_failures+warning，行保持 pending ──
  ...
  const snapWF = await snap(runWF, '13800000001', '张三'); // snap 走主 mod（正常回写），仅 confirm 走 modWF
  ...
  serverWF.close();
  ok('M-new：阶段三 UPDATE 影响 0 行 → 返回 write_failures+warning、行保持 pending、不误报全成功（逻辑未动）');

  // ── [10d] M / M-2（⑤c）端到端：WRITE_FAILED 卡死 pending → NEEDS_RECONCILE；force 放行 + 旧行转 failed+SUPERSEDED_BY_FORCE 退出阻断 ──
  ...
  ok('M/M-2（⑤c）端到端：WRITE_FAILED 卡死 pending → NEEDS_RECONCILE；force 放行 + 旧行转 failed+SUPERSEDED_BY_FORCE 退出阻断 + 之后判 ALREADY_PUSHED（reconcile 生命周期闭合，逻辑未动）');

  // ── [10e] M-1（⑤c）僵尸 pending push reaper：造超阈值 plain pending → 跑 reapStalePendingPushes → 补 WRITE_FAILED:stale_reaped 且仍 pending ──
  ...
  ok('M-1（⑤c）僵尸 pending push reaper：超阈值 plain pending → 补 WRITE_FAILED:stale_reaped 仍 pending（可 force 恢复）+ 三守卫（新鲜不误伤/已标记不重复/终态不动）+ 幂等（逻辑未动）');

  // ── [11] invalidStaffIdList 命中 → failed（区别于反查 skipped）；两阶段留痕从 pending→failed ──
  const run11 = await freshRun();
  const snap004 = await snap(run11, '13800000004', '测试无效员工'); // uid_invalid_staff
  ...
  const snap001b = await snap(run12, '13800000001', '张三');
  ...
  const snap001c = await snap(run13, '13800000001', '张三');
  ...
  const snap001d = await snap(run14, '13800000001', '张三'); // uid0001
  const snap008 = await snap(run14, '13800000008', '同一人别的号');  // 也映射 uid0001
  ...
  // ── [17] 参数校验：preview recipients 空/超限；confirm recipients 超限 ──
  r = await call('POST', `/api/periodic-tasks/runs/${runSuccess}/push/preview`, adminTok, { recipients: [] });
  assert.strictEqual(r.status, 400);
  assert.strictEqual(r.body.code, 'RECIPIENTS_REQUIRED');
  r = await call('POST', `/api/periodic-tasks/runs/${runSuccess}/push/preview`, adminTok, { recipients: Array(21).fill({ name: '甲', phone: '13800000001' }) });
  assert.strictEqual(r.status, 400);
  ...
  assert.strictEqual(r.status, 400);
  assert.strictEqual(r.body.code, 'TOO_MANY_PHONES');
  ok('参数校验：preview recipients 空→RECIPIENTS_REQUIRED / 超 20→TOO_MANY_PHONES；confirm recipients 超 20→TOO_MANY_PHONES');

  // ── [18] requireAdmin：无 token → 401（preview + confirm）──
  r = await call('POST', `/api/periodic-tasks/runs/${runSuccess}/push/preview`, null, { recipients: [{ name: '甲', phone: '13800000001' }] });
...
async function main() {
  ...
  console.log('  覆盖：resolvePushRecipient(phone→userid 无姓名/在职·getUserDetail 已删) + H-new evaluatePushIdempotency单元(含⑤c NEEDS_RECONCILE区分) + compareRecipientSnapshot/normalizeForCompare 已删断言 + 可推送前置(5态) + 预览(录入姓名透传+入参校验+无副作用)+M-1去重 + 确认发送(H-1快照必带/userid硬gate MISMATCH/姓名录入不做drift/H-3两阶段/H-3幂等ALREADY_PUSHED+force/H-new并发pending PUSH_IN_PROGRESS+ROLLBACK/M-new回写失败write_failures+warning+保持pending/⑤c M-2 force supersede/⑤c M-1 reapStalePendingPushes+三守卫+幂等/发送异常不卡pending/M-1双去重/invalid_staff failed/no_config 502/绑定run_id) + 参数校验 + requireAdmin + 记录列表 + recipient_name_snapshot=录入姓名');
  server.close();
  db.close();
