...
(async () => {
  ...
  // ④ 建单 0 业务方 → 400
  const c4 = await reqJson('POST', '/api/corrections', { source_system: 'BMS', location_info: '无业务方', correction_type: 'single' }, ADMIN);
  ok(c4.status === 400 && c4.body.code === 'MISSING_REQUIRED_FIELDS', '建单 0 业务方 → 400 MISSING_REQUIRED_FIELDS');
  const c4b = await reqJson('POST', '/api/corrections', { source_system: 'BMS', location_info: '全空名', correction_type: 'single', requesters: [{ name: '  ' }, { name: '' }] }, ADMIN);
  ...
  const cMany = await reqJson('POST', '/api/corrections', { source_system: 'BMS', location_info: '超量', correction_type: 'single', requesters: Array.from({ length: 21 }, (_, i) => ({ name: '人' + i })) }, ADMIN);
  ok(cMany.status === 400 && cMany.body.code === 'TOO_MANY_REQUESTERS', 'M-1：requesters>20 → 400 TOO_MANY_REQUESTERS');
  const cLongName = await reqJson('POST', '/api/corrections', { source_system: 'BMS', location_info: '超长名', correction_type: 'single', requesters: [{ name: '啊'.repeat(101) }] }, ADMIN);
