/**
 * 数据协作·「数据范围说明」多文件**展示**回归（生产 #53 hotfix）
 *
 * 用法：node scripts/test-collab-data-scope-display-playwright.js
 * 前置：本地 server 已启动（localhost:3000）
 *
 * ── 为什么单独立这个套件 ────────────────────────────────────────────────
 * data_scope 自 v1.77.0「多文件上传 M1/M3」起写端就是多文件（前端 `<input multiple>` +
 * 后端循环入库 ≤5 个），但详情页渲染一直是 `.filter(...)[0]` —— **只显示首个**。
 * 生产 #53 因此漏文件：库里 2 条 active（附件1/附件2），开发人员只看得到 1 条。
 *
 * 既有 `test-collab-multifile-m3-playwright.js` 自述「纯客户端」，T1-T9 全部停在
 * **选择/预览层**（input.files 去重、超量截断、扩展名过滤），没有一条走到"落库 → 详情页"，
 * 所以上传层全绿了一年多，展示层的缺口始终无人拦 —— 典型的「层内全绿 ≠ 功能可用」。
 * 本套件补的正是那一段：**写端存了 N 个，读端就必须显示 N 个**。
 *
 * 覆盖：
 *   T1 两个 data_scope（复刻生产 #53 形态）→ 两个文件名都出现 + 「共 2 个文件」计数 + 逐项 href 对应
 *   T2 排序：**夹具刻意让接口逆序返回**（大 id 的 created_at 更晚 → ORDER BY created_at DESC 先返回），
 *      页面仍须按 id 升序展示 —— 这样删掉前端 sort 本条就会红（否则是偶然通过·codex 审 M-2）
 *   T3 混合夹具：合法 active + 非法路径 active + 非 active + 其他类型 + status 为 NULL 的历史行
 *      → 只展示应展示的，非法项出警告且**不带链接**，计数只数展示项（codex 审 M-3）
 *   T4 文件名含单引号/双引号/尖括号 → 属性边界不被突破、不产生额外元素（codex 审 M-1）
 *   T5 单个 data_scope → 正常显示且**不出现**计数提示（避免给绝大多数单据添噪声）
 *   T6 零个 data_scope → 显示占位「—」
 *   + 每个页面都注册 console error 监听（codex 审 L-1：原先只 T1 注册却声称"全程 0 error"）
 *
 * 夹具真实性：单据走 `createPendingFixture()` 真实建单 API；附件按既有 e2e 范式直接 INSERT，
 *   形态（同单多条 active data_scope、attachment_seq 递增、无 supersede）与生产 #53 逐项一致。
 * ⚠️ 覆盖边界（如实声明）：直插附件记录**不创建实体文件**，故本套件只证明"展示层把记录都渲染出来"，
 *   **不证明**下载真的可用（实体文件缺失/路径非法不在本套件射程内）。
 */
'use strict';
...
// 读「数据范围说明」那一格（kv 结构：label 的下一个 sibling 是 value）
async function readDataScopeCell(page) {
    return await page.evaluate(() => {
        const labels = [...document.querySelectorAll('.detail-kv-label')];
        const lab = labels.find(l => (l.textContent || '').trim() === '数据范围说明');
        if (!lab) return null;
...
async function main() {
    ...
    try {
        ...
        // ═══════════════════════════════════════════════════════════════
        // T1 + T2：两个 data_scope → 都显示 + 计数 + 逐项 href 对应 + 排序有判别力
        // ═══════════════════════════════════════════════════════════════
        {
            console.log('\n── T1/T2：两个数据范围文件 → 都显示 + 计数 + 排序（接口逆序前提） ──');
            const fxA = await fx.createPendingFixture();
            created.push(fxA.id);
            const stamp = Date.now();
            const nameA1 = `DS一-需确认员工身份信息-${stamp}.xlsx`;
            const nameA2 = `DS二-需确认员工身份信息-${stamp}.xlsx`;
            ...
            const fileA1 = `collab/test/ds-A1-${stamp}.xlsx`;
            const fileA2 = `collab/test/ds-A2-${stamp}.xlsx`;
            ...
            const apiOrder = await page.evaluate(async (cid) => {
                ...
                return (j.attachments || []).filter(a => a.attachment_type === 'data_scope').map(a => a.id);
            }, fxA.id);
            ...
            const cell = await readDataScopeCell(page);
            must(!!cell, `能定位到「数据范围说明」kv 格`);
            if (cell) {
        ...
        // ═══════════════════════════════════════════════════════════════
        {
            ...
            created.push(fxD.id);
            const s = Date.now();
            const okName = `DS合法-${s}.xlsx`;
            const nullStatusName = `DS历史缺省态-${s}.xlsx`;
            const badPathName = `DS非法路径-${s}.xlsx`;
            const supersededName = `DS已废止-${s}.xlsx`;
            ...
            // 其他类型：不应出现在本格
            await insertAttachment(fxD.id, otherTypeName, 1, { type: 'example_xlsx' });
        ...
        // ═══════════════════════════════════════════════════════════════
        {
            ...
            //   或有人把属性改成未转义拼接，这里立刻红。
            const trickyName = `DS'单引号"双引号<script>-${Date.now()}.xlsx`;
        ...
        // ═══════════════════════════════════════════════════════════════
        {
            console.log('\n── T5：单个数据范围文件 → 显示且无计数噪声 ──');
            const fxB = await fx.createPendingFixture();
            created.push(fxB.id);
            const nameB = `DS单文件-${Date.now()}.xlsx`;
    ...
    if (fail > 0) {
        console.log('  ❌ data_scope 展示回归存在失败项');
    ...
    console.log('  🎉 data_scope 多文件展示回归全部通过');
}
