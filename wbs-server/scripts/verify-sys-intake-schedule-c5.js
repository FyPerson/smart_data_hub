...
//
// 覆盖（真实 HTTP + 条件落库直测）：
//   [R] request_tech_consult（待受理·requireIntakeLiaison·选技术负责人+首发）：
//       受理人(13)/admin 200 + tech_lead_id/name 派生 + request_event_id 设 + 首发 sent + timeline(note/request_tech_consult/summary含技术负责人)
//       + 非白名单 tech_lead 400 / 缺 tech_lead_id 400 / 非受理人(dev5) 403 中间件 / 非待受理态 409
//   [V] 请求版本 + 结果归属（§6·codex 128-M/130-H）：换人/同人连发生成新 request_event_id + 重置投递字段（sent_by/read_at/error 清）；
//       首发失败注入 → failed + sent_by 非空（§8.3 failed⟹sent_by）；sent⟹notified_at+message_key+sent_by
//   [S] 拒过期回写（条件 request_event_id）：直测 recordSysTechLeadNotify 旧 request_event_id → changes=0（不覆盖新版本投递态）
...
async function main() {
  ...
  // ═══ [R] request_tech_consult（待受理·选技术负责人+首发）═══
  {
    sendBehavior = { mode: 'ok' };
    // 受理人(13) request tech_lead(7) → 200 + 派生 name + request_event_id + 首发 sent
    ...
    assert.strictEqual(r.body.tech_lead_notify_status, 'sent', '首发 sent（stub ok）');
    assert.ok(lastSent && Number(lastSent.userId) === 7, '首发实际发给技术负责人(7)');
    // 状态旁路：不改 status（仍待受理）
    ...
    // §8.3：sent⟹notified_at+message_key+sent_by 非空·read_at 空
    assert.strictEqual(Number(row.tech_lead_notify_sent_by), 13, 'sent_by=操作者13');
    assert.ok(row.tech_lead_notified_at, 'sent⟹notified_at 非空');
    assert.strictEqual(row.tech_lead_notify_message_key, 'stub-tl', 'sent⟹message_key 非空');
    assert.strictEqual(row.tech_lead_read_at, null, 'read_at 每次新投递重置 NULL');
    // timeline note + action_code + summary 含技术负责人
    ...
    // 非待受理态(待指派) → 409
    id = await createIssue('feature');   // 待指派+ir0
    r = await call('POST', `/api/sys-issues/${id}/request-tech-consult`, adminTok, { tech_lead_id: 7 });
    assert.strictEqual(r.body.code, 'REQUEST_TECH_CONSULT_STATUS_INVALID', '非待受理 409');
    ok('[R] request_tech_consult：受理人/admin 200(派生name+request_event_id+首发sent+§8.3字段) + 非白名单/缺id 400 + 非受理人 403 + 非待受理 409');
  }
  ...
  // ═══ [V] 请求版本 + 结果归属（换人/连发新版本+重置·首发失败注入）═══
  {
    ...
    // 换人（7→7 已测同人；此处验重置：先 fail 版本，再新 request 重置为 sent）
    // 首发失败注入 → failed + sent_by 非空（§8.3 failed⟹sent_by）
    sendBehavior = { mode: 'fail' };
    id = await createIssue('feature'); await seedIntake(id);
    r1 = await call('POST', `/api/sys-issues/${id}/request-tech-consult`, liaisonTok, { tech_lead_id: 7 });
    assert.strictEqual(r1.body.tech_lead_notify_status, 'failed', '首发失败 → failed');
    ...
    // 再 request（重置）+ 首发成功 → sent（新版本覆盖旧 failed）
    sendBehavior = { mode: 'ok' };
    r2 = await call('POST', `/api/sys-issues/${id}/request-tech-consult`, liaisonTok, { tech_lead_id: 7 });
    assert.strictEqual(r2.body.tech_lead_notify_status, 'sent', '重新发起+首发成功 → sent（覆盖旧 failed·新版本）');

    // 重置观察（MED-7）：预置旧投递字段 → request 失败 → 断言旧 message_key/read_at 被清（重置生效·非残留）
    sendBehavior = { mode: 'fail' };
    id = await createIssue('feature'); await seedIntake(id);
    ...
    assert.strictEqual(row.tech_lead_notify_message_key, null, '重置清旧 message_key（failed 后仍 null）');
    assert.strictEqual(row.tech_lead_read_at, null, '重置清旧 read_at');
    assert.strictEqual(row.tech_lead_notify_status, 'failed', '本次失败落 failed');

    // HIGH-3：send ok 但漏 message_key → 降级 failed(message_key_missing)（§8.3 sent⟹message_key）
    sendBehavior = { mode: 'ok_nokey' };
    id = await createIssue('feature'); await seedIntake(id);
    r1 = await call('POST', `/api/sys-issues/${id}/request-tech-consult`, liaisonTok, { tech_lead_id: 7 });
    assert.strictEqual(r1.body.tech_lead_notify_status, 'failed', 'send ok 但无 message_key → 降级 failed（HIGH-3）');
  ...
  // ═══ [S] 拒过期回写（条件 request_event_id·直测 recordSysTechLeadNotify changes=0）═══
  {
    ...
    assert.strictEqual(row.tech_lead_notify_status, 'sent', '新版本投递态未被过期回写覆盖（仍 sent）');
  ...
  // ═══ [RS] resend-tech-consult（admin∨受理人∨建单人·expected_request_event_id）═══
  {
    ...
    // MED-6：失败 error 是安全码（no_phone）不含 SQLite/内部信息
    sendBehavior = { mode: 'fail' };
    id = await createIssue('feature'); await seedIntake(id);
    r = await call('POST', `/api/sys-issues/${id}/request-tech-consult`, adminTok, { tech_lead_id: 7 });
