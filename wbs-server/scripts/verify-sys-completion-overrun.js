...
async function main() {
  ...
  //   是本条注释在提醒下一个改动者。
  {
    ...
    const EXPECTED_STATUS_WRITE_SITES = [
      ...
      //   第 9 处）。
      { anchorLine: 14641, reachable: false, desc: '批量发布执行 UPDATE：字面量写「已上线」（同一写点，flip UPDATE 语句本体未变；行号历经多批注释/重排/纯函数抽取累计下移，见上方各批注释；R-C4 首批 +54、codex 472 审收口批再 +11、476 收口批 DDL CHECK 再 +5——本锚已第 9 次被无关改动追平——2026-08-27 一天之内被本模块**连续三次**追平：加 4 个 release 侧派生列(+24 行) → 加 my_release_exec_pending(+25 行) → 论证后撤回 release_creator_active(-11 行)。同一个锚一天校准三遍，backlog #25③「绝对行号锚改结构锚」已不是"应该做"而是"再不做，每次动 index.js 都要重来一遍"，backlog #25③「绝对行号锚改结构锚」实例再 +1，该债已具备立即处理的充分理由）' },
    ];
