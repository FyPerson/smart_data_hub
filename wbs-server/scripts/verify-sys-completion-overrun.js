...
async function main() {
  ...
  //   是本条注释在提醒下一个改动者。
  {
    ...
    const EXPECTED_STATUS_WRITE_SITES = [
      ...
      { anchorLine: 4265, reachable: true, gateKind: 'runWGate', desc: 'runWGate 内唯一 UPDATE——targetStatus 变量，feature 决策树 ⑤⑤变体【新skip_system】⑥⑦ 分支据 SF.SYS_VERIFY_STATUSES/SF.SYS_LIAISON_TEST_STATUSES 可解析为待验证/待对接测试；§14 闸门（enteringForward && issueType===feature 分支）在本 UPDATE 之前执行，无理由不放行（同一写点，UPDATE 语句字面量未变；行号随 2026-09-06 验收说明附件批 resolveEvidenceAttachmentIds helper 插入 +35 行·grep 实测·第 17 次校准；2026-09-07 补丁 T/V 累计再 +5·第 21 次校准；2026-09-09~10 提交修正与config指派OA守卫 C3/C3b/C3c/C4 四批（POST /sys-issues/:id/submit/amend 端点+computeDeliveryRev 版本锁+探针 P16 等，均落在本写点**之前**）累计 +50 行=**第 24 次校准**，grep 实测 4265，②处实测计数仍恰 8——非新增第 9 处，是同一处 runWGate UPDATE 随前文代码量增长而下移）' },
      { anchorLine: 4814, reachable: false, desc: '先行上线翻牌内核 attemptFastReleaseFlipInTxn：字面量写「已上线」，WHERE 限定 type=bug，与 feature 专属闸门结构上无交集（同一写点，行号随 2026-09-06 验收说明附件批 helper 插入整体下移 +35·grep 实测·第 17 次校准；2026-09-07 补丁 T/V 累计再 +5·第 21 次校准；2026-09-09~10 提交修正与config指派OA守卫 C3/C3b/C3c/C4 四批累计 +52=**第 24 次校准**，grep 实测 4814）' },
      { anchorLine: 6632, reachable: true, gateKind: 'exempt', desc: 'sysIssueTransition 通用引擎唯一 UPDATE——toStatus 变量，服务全部声明式 transition；feature 流里能落到目标两态的边只有①核实的 liaison_test_pass 一条，该边已撤闸（显式豁免，见下方子断言核实其代码块不引用理由闸函数；行号随 2026-09-06 验收说明附件批 helper+accept/return 追加逻辑下移 +84·grep 实测·第 17 次校准；2026-09-07 补丁 T/V 累计再 +9·第 21 次校准；2026-09-09~10 提交修正与config指派OA守卫 C3/C3b/C3c/C4 四批累计 +83=第 24 次校准，grep 实测 6611；2026-09-10 待对接测试态放开提交修正 D-L1 决策记录批——case 「liaison_test_pass」内插入对接测试通过版本锁校验（expected_delivery_rev 存在即校验+409 比对）与 payload_json 条件写 delivery_rev，均落在本写点**之前**，净 +21 行=**第 26 次校准**，grep 实测 6632）' },
      { anchorLine: 7347, reachable: false, desc: '建单 path A 占位状态 UPDATE：finalStatus 恒为受理门初始态（resolveSysInitialStatusForCreate 落态）或「开发中」，结构上不可能是待对接测试/待验证（行号随 2026-09-06 验收说明附件批下移 +85·grep 实测·第 17 次校准；2026-09-07 补丁 T/V 累计再 +9·第 21 次校准；2026-09-09~10 提交修正与config指派OA守卫 C3/C3b/C3c/C4 四批累计 +83=第 24 次校准，grep 实测 7324；2026-09-10 待对接测试态放开提交修正 D-L1 决策记录批——case 「liaison_test_pass」版本锁插入（本条写点**之前**）净 +23=**第 26 次校准**，grep 实测 7347）' },
      { anchorLine: 7612, reachable: false, desc: '/assign 端点 UPDATE：targetStatus 恒为 SF.SYS_DEV_STATUSES[type][0]（开发中），结构上不可能是待对接测试/待验证（行号随 2026-09-06 验收说明附件批下移 +85·grep 实测·第 17 次校准；2026-09-07 补丁 T/V 累计再 +16·第 21 次校准；2026-09-09~10 提交修正与config指派OA守卫 C3/C3b/C3c/C4 四批累计 +84=第 24 次校准，grep 实测 7626；2026-09-10 #56 执行方式契约整组下线，assign 端点内 exec_mode/vendor_name 必填校验与 UPDATE 字段拼接一并删除，净减 36 行=第 25 次校准，grep 实测 7590；同日待对接测试态放开提交修正 D-L1 决策记录批——case 「liaison_test_pass」版本锁插入（本条写点**之前**）净 +22=**第 26 次校准**，grep 实测 7612）' },
